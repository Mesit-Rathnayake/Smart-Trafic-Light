...
def optimize_model():
    ...
    # FIXED: Changed reward_batch to batch_reward on the RHS to resolve UnboundLocalError
...
def run():
    ...
    # Robust Path Setup: Locate 'sumo' dynamically
    sumo_binary = os.path.join(os.environ['SUMO_HOME'], 'bin', 'sumo')

    # We explicitly force SUMO to use 'peak.rou.xml' to avoid configuration mismatches!
    sumo_cmd = [
        sumo_binary,
        "-c", "simulation.sumocfg",
        "--route-files", "peak.rou.xml",
        "--no-step-log", "true",
        "-W"
    ]
    ...
    # Open separate log file for peak metrics
    with open('training_log_peak.csv', 'w', newline='') as file:
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            try:
                ...
                if epoch_reward > best_reward:
                    ...
                    # Save checkpoint for optimal peak performance weights
                    torch.save(policy_net.state_dict(),
                               "trained_brain_peak_best.pth")
                    print(
    ...
    print(f"\n🎉 Peak Multi-Epoch Training Complete.")
    print(f"🏆 Your most optimal Peak AI was saved as 'trained_brain_peak_best.pth'!")
    print("📊 Training metrics successfully logged to 'training_log_peak.csv'!")
