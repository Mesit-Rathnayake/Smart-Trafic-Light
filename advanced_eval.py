...
# TOGGLE BRAIN HERE: Set to "trained_brain_peak_best.pth" or "trained_brain_offpeak_best.pth"
MODEL_PATH = "trained_brain_offpeak_best.pth"

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
policy_net = DQN(37, 4).to(device)

# Load the specified brain
...
def run_test():
    # Robust Path Bypass: Dynamically target the 'sumo-gui' binary
    ...
    try:
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            for _ in range(10):
                ...
                for lane in incoming_lanes:
                    ...
                    # 4. Track Queues
                    current_step_queue += traci.lane.getLastStepHaltingNumber(
                        lane)
        ...
        print(f"\n" + "="*50)
        print(f"🎓 PHD-LEVEL EVALUATION REPORT ({MODEL_PATH})")
        print(f"="*50)
        print(
            f"🚦 Total Accumulated Wait Time: {total_waiting_time:,.2f} seconds")
        print(
            f"🚗 Intersection Throughput:     {total_throughput} vehicles cleared")
        print(
            f"🛑 Maximum Queue Recorded:      {max_queue_length} vehicles at once")
